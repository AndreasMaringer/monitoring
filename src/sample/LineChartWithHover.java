...
public class LineChartWithHover extends Application {
    ...
    public void start(Stage stage) {
        final LineChart lineChart = new LineChart(new NumberAxis(), new NumberAxis(), FXCollections.observableArrayList(
                        new XYChart.Series("My portfolio", FXCollections.observableArrayList(plot(23, 14, 15, 24, 34, 36, 22, 45, 43, 17, 29, 25))
                        )
                )
        );
    ...
    public ObservableList<XYChart.Data<Integer, Integer>> plot(int... y) {
        ...
        int i = 0;
        while (i < y.length) { //y.length anzahl der y Werte
            final XYChart.Data<Integer, Integer> data = new XYChart.Data<>(i + 1, y[i]);
            data.setNode(new HoveredThresholdNode((i == 0) ? 0 : y[i - 1], y[i]));// Bedingung: (i == 0) ?(wahr) dann = 0 :(sonst) y[i-1] bei i>0, 2.Parameter: y[i]
            dataset.add(data);
            i++;
        }
        return dataset;
